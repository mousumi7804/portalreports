...
namespace DataLayer
{
    public class RptDocChemMapDL
    {
        public RptDocChemMapDL()
        {
            //
        ...
        public String populateReportFromSP(string mont,int mno)
        {
            ...
            Common.OpenConnection();
            string sql = "delete from rpt_doctorchemistmapping where monthno=" + mno + " and year='" + curyear.ToString() + "'";
            SqlCommand cmd = new SqlCommand(sql, Common.conn);
            cmd.ExecuteNonQuery();

            using (SqlCommand cmd1 = new SqlCommand("SaveDoctorChemistMappingReportFromDT"))
            {
            ...
            ret = "Execution completed for Doctor Chemist mapping report";
            return ret;
        }
    }
}