using System;
using System.Collections.Generic;
using System.Configuration;
using System.Data.SqlClient;
using System.Linq;
using System.Web;

/// <summary>
/// Summary description for Common
/// </summary>
/// 

namespace model
{
    public class Common
    {
        public Common()
        {
        }
        public static SqlConnection conn;
        public static string[] monthlist = new string[] { "Select", "January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December" };
        public static void OpenConnection()
        {
            //String strconn = ConfigurationManager.ConnectionStrings["MendineMasterdummyConnectionString"].ConnectionString;
            String strconn = ConfigurationManager.ConnectionStrings["ConnectionString_master"].ConnectionString;
            
            conn = new SqlConnection(strconn);
            conn.Open();
        }

        public static void CloseConnection()
        {
            conn.Close();
        }

        public static int GetPositionof(string value)
        {
            int ret = 0;

            ret = Array.FindIndex(monthlist, m => m == value);

            return ret;
        }

        public static int GetInt(object value)
        {
            int retVal = 0;
            string val = Convert.ToString(value);

            if (val != null && val != "")
            {
                bool found = false;
                for (int i = 0; i < val.Length && (!found); i++)
                {
                    char ch = val[i];
                    if (char.IsDigit(ch) == false)
                        found = true;
                }

                if (!found)
                    retVal = Convert.ToInt32(value);
            }

            return retVal;
        }

        public static int GetLastdayOfMonth(DateTime curdate)
        {
            int lastday = 30;

            DateTime firstdate = DateTime.Parse(curdate.Month + "-" + "01-" + curdate.Year);
            DateTime firstdatenextmonth = firstdate.AddMonths(1);
            DateTime lastdateofmonth = firstdatenextmonth.AddDays(-1);
            lastday = lastdateofmonth.Day;

            return lastday;
        }

        public static string GetString(object value)
        {
            string retVal = "";
            if (value != null && value != "")
                retVal = Convert.ToString(value);

            return retVal;
        }

        public static bool GetBool(object value)
        {
            bool retValue = false;

            if (value != null && value != "")
            {
                try
                {
                    retValue = Convert.ToBoolean(value);
                }
                catch (Exception ex)
                {
                    retValue = false;
                }
            }
            return retValue;
        }

        public static decimal GetDecimal(object value)
        {
            decimal retValue = 0;

            if (value != null && value != "")
            {
                try
                {
                    retValue = Convert.ToDecimal(value);
                }
                catch (Exception ex)
                {
                    retValue = 0;
                }
            }
            return retValue;
        }

        public static string GetTrimmedString(string value)
        {
            string retval = "";

            if (value != null)
                retval = value.Trim();

            return retval;
        }


        public static int GetMonthNo(string monthname)
        {
            int ret = 0;
            bool blnfound = false;
            for (int i = 0; i < monthlist.Length && (!blnfound); i++)
            {
                if (monthlist[i].ToUpper() == monthname.ToUpper())
                {
                    ret = i;
                    blnfound = true;
                }
            }

            return ret;
        }
    }
}